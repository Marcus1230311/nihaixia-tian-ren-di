  ...
  return (
    <>
      <section className="hero-shell">
        ...
        <p>沿著《易經》、八字、河洛、人紀針灸、《傷寒論》與《金匱要略》的引導路徑研讀，也可從卦象、干支、經脈、穴位、六經、病類、病證、方劑與藥物交叉查找。每一筆整理都保留來源與清楚的內容界線。</p>
        <div className="hero-actions"><Link href={`/lessons/${lessons[0].route.join("/")}/`}>從易經開始</Link><Link href="/lessons/tianji/bazi/01-tiangan-dizhi/">進入八字基礎</Link><Link href="/lessons/tianji/heluo/01-hetu-luoshu/">認識河圖洛書</Link><Link href="/lessons/renji/acupuncture/01-meridians/">認識十二正經</Link><Link href="/lessons/renji/shanghan/01-overview/">進入傷寒六經</Link><Link href="/lessons/renji/jingui/01-overview/">進入金匱雜病</Link></div>
      </section>
      <SearchPanel />
      <section className="platform-grid" aria-labelledby="model-title">
        <div><p className="kicker">研讀架構</p><h2 id="model-title">從天紀走向人紀的共享知識</h2><p>目前有 {lessons.length} 篇引導課程、{trigrams.length} 個八卦、{hexagrams.length} 個六十四卦、{stems.length} 個天干、{branches.length} 個地支、{meridians.length} 條正經與 {acupoints.length} 個標準穴位，以及 {shanghanChannels.length} 個傷寒診斷六經、{conditions.length} 個金匱病類與 {formulas.length} 張代表方。五行、陰陽、方劑與藥材身份可跨模組重用。</p></div>
